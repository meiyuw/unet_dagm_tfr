...
def _crop_and_concat(inputs, residual_input):
    ...
    return tf.concat([inputs, residual_input], axis=3)
...
class InputBlock(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        mp = self.maxpool(out)
        #print('Input block',mp.shape,out.shape)
        return mp, out
...
class DownsampleBlock(tf.keras.Model):
    ...
    def call(self, inputs):
        #print('Downsample shape intput:',inputs.shape)
        out = self.conv1(inputs)
        #print('Downsample shape intput:',out.shape)
        out = self.conv2(out)
        #print('Downsample shape intput:',out.shape)
        mp = self.maxpool(out)
        #print('Downsample shape output:',out.shape, mp.shape)
        return mp, out
...
class BottleneckBlock(tf.keras.Model):
    def __init__(self, filters, idx):
        ...
        with tf.name_scope('bottleneck_block_{}'.format(idx)):
            self.conv1 = tf.keras.layers.Conv2D(filters=filters,
                                                kernel_size=(3, 3),
                                                padding = 'same',
                                                activation=tf.nn.relu)
...
class UpsampleBlock(tf.keras.Model):
    def __init__(self, filters, idx):
        ...
        with tf.name_scope('upsample_block_{}'.format(idx)):
            ...
            self.conv2 = tf.keras.layers.Conv2D(filters=filters,
                                                ...
                                                padding='same',
                                                activation=tf.nn.relu)
            self.conv_transpose = tf.keras.layers.Conv2DTranspose(filters=filters,
                                                                  kernel_size=(2, 2),
                                                                  strides=(2, 2),
                                                                  padding='same',
                                                                  activation=None)
    ...
    def call(self, inputs, residual_input):
        #print('Upsampling input:',inputs.shape, residual_input.shape)
        out = self.conv_transpose(inputs)
        #print(out.shape,residual_input.shape)
        out = _crop_and_concat(out, residual_input)
        #print('concat shape:',out)
        out = self.conv1(out)
        #print('Upsampling conv1:',out.shape)
        out = self.conv2(out)
        #print('upsample block shape:',out.shape)
        #print('Upsampling conv2:',out.shape)
        #out = self.conv_transpose(out)
        #print('Upsampling onv_transpose:',out.shape)
        return out
