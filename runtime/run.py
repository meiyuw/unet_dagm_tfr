...
from runtime.losses import partial_losses, build_total_loss
...
def train(params, model, dataset, logger):
    ...
    def train_step(features, labels, warmup_batch=False):
        with tf.GradientTape() as tape:
            ...
            #print('train_step shape:',output_map.shape, labels.shape)
            loss = build_total_loss(output_map, features, labels)
...
def evaluate(params, model, dataset, logger, restore_checkpoint=True):
    ...
    logger.log(step=(),data={"train_total_loss": float(total_loss.result())})
